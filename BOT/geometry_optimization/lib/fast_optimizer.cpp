...
double zeta(double EW, double al){
    double cost = pow(fabs(EW),al);//pow(pow(fabs(EW),0.5)*(1.0+fabs(EW)),al);
    return cost;
}
...
void optimize(int DIMENSION, int NUMSITES, int adj[][3], double EW[][3], double EL[][3], double XX[], double al, double beta = 1.0){
    ...
    for(i=k1-1;i>=0;i--){
        ...
        n1 = adj[i][1];
        n2 = adj[i][2];
        q0 = zeta(EW[i][0],al)*pow(EL[i][0]+tol,beta-2.0);
        q1 = zeta(EW[i][1],al)*pow(EL[i][1]+tol,beta-2.0);
        q2 = zeta(EW[i][2],al)*pow(EL[i][2]+tol,beta-2.0);
...
double length(int DIMENSION, int NUMSITES, int adj[][3],  double EW[][3], double EL[][3], double XX[], double al, double beta = 1.0) {
    ...
    for(i=0;i<k1;i++){
        ...
        if(n0<i2){
            dist(n0,i2);leng+=zeta(EW[i][0],al)*pow(t,beta);EL[i][0]=t;n0-=NUMSITES;
            if(n0>=0)for(j=0;j<3;j++)if(adj[n0][j]==i2){EL[n0][j]=t;break;}
        }
        if(n1<i2){
            dist(n1,i2);leng+=zeta(EW[i][1],al)*pow(t,beta);EL[i][1]=t;n1-=NUMSITES;
            if(n1>=0)for(j=0;j<3;j++)if(adj[n1][j]==i2){EL[n1][j]=t;break;}
        }
        if(n2<i2){
            dist(n2,i2);leng+=zeta(EW[i][2],al)*pow(t,beta);EL[i][2]=t;n2-=NUMSITES;
    ...
    while(!BPQ.empty()){
        ...
        while(!BPQ2.empty()){
            ...
            for(d=0;d<DIMENSION;d++){
                XX[(b+NUMSITES)*DIMENSION+d] = XX[k1*DIMENSION+d]+0.001*((double)rand()/(double)RAND_MAX/2 + 0.5);//(double)((rand() % (4*NUMSITES) + 4*NUMSITES)/(8*NUMSITES));
            }
...
extern "C"
double iterations(int *iter, int DIMENSION, int NUMSITES, int adj[][3], double EW[][3], double demands[], double XX[], double al, double improv_thres = 1e-7, double beta = 1.0){
